...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight( Picture pic)
    {
        ...
        {
            int temp;
            for(int x = 0; x < picWidth / 2; x++)
            {
                temp = pixels[y][x];

                pixels[y][x] = pixels[y][pic.getWidth() - 1 - x];
                pixels[y][pic.getWidth() - 1 - x] = temp;
            }
        }
        Picture grayPic = new Picture(pixels);
        return grayPic;
    }
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        //int rotateHeight = coolPic.getHeight();
        //int rotateWidth = coolPic.getWidth();
        ...
        for(int y = 0; y < picHeight; y++)
        {
            for(int x = 0; x < picWidth; x++)
            {
                ...
                pixelsRotate[x][picHeight - 1 - y] = pixels[y][x];


            }
        }

        Picture coolPic = new Picture(pixelsRotate);

        return coolPic; //just so draft will compile. You will change it in the final
    }
